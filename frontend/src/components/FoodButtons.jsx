...
export default function FoodButtons() {
  ...
  const [description, setDescription] = useState(
    "welcome, choose the food you like."
  );
  ...
  const handleCultureClick = async (e) => {
    ...
    if (stage === 2) {
      ...
      try {
        const response = await fetch(
          `/api/recipes?name=${encodeURIComponent(selectedButton)}`,
          {
            method: "GET",
            headers: {
        ...
        if (data) {
          ...
          const ingredientsStr = data.ingredients
            .map((ing) => {
              count++;
              return `${count}-${ing.name}: ${ing.amount} ${ing.unit}`;
            })
            .join("\n");
        ...
        console.log("error getting the recipe");
      }
      return;
    }
